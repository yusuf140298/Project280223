                ...
                <tbody>
                    @forelse($reimburs as $row)
                        ...
                        <tr>
                            ...
                            <td class="border px-4 py-2 text-center">@if($row->status == 1) <button wire:click="detail({{ $row->id }})" class="bg-purple-500 hover:bg-purple-700 text-white py-2 px-3">Detail</button>@elseif($row->status == 2) <span class="text-red-500">Reject</span>@elseif($row->status == 3) <span class="text-green-500">Claim</span>@else <span class="text-yellow-500">Waiting</span> @endif</td>
